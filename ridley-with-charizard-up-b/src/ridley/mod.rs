...
// STATUS
// Main
unsafe extern "C" fn ridley_upb_status_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    WorkModule::set_float(fighter.module_accessor, 5.5, *FIGHTER_STATUS_SUPER_JUMP_PUNCH_WORK_FLOAT_CONST_AIR_ACCEL_Y);
    ...
    fighter.sub_shift_status_main(L2CValue::Ptr(ridley_upb_status_main_loop as *const () as _))
}

// Main Loop
unsafe extern "C" fn ridley_upb_status_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
...
// Pre
unsafe extern "C" fn ridley_upb_status_pre(fighter: &mut L2CFighterCommon) -> L2CValue {
...
// End
unsafe extern "C" fn ridley_upb_status_end(fighter: &mut L2CFighterCommon) -> L2CValue {
...
// ACMD
unsafe extern "C" fn ridley_upb_acmd(agent: &mut L2CAgentBase) {
...
unsafe extern "C" fn ridley_upb_air_acmd(agent: &mut L2CAgentBase) {
    frame(agent.lua_state_agent, 4.0);
    if macros::is_excute(agent) {
        damage!(agent, *MA_MSC_DAMAGE_DAMAGE_NO_REACTION, *DAMAGE_NO_REACTION_MODE_ALWAYS, 0);
    }
...
unsafe extern "C" fn ridley_upb_acmd_fx(agent: &mut L2CAgentBase) {
    if macros::is_excute(agent) {
...
unsafe extern "C" fn ridley_upb_air_acmd_fx(agent: &mut L2CAgentBase) {
    if macros::is_excute(agent) {
...
pub fn install() {
    Agent::new("ridley") 	// Replace "kirby" with character you want to modify, using their internal code name.
		// List of functions:
		.status(Main, *FIGHTER_STATUS_KIND_SPECIAL_HI, ridley_upb_status_main)
        .status(Pre, *FIGHTER_STATUS_KIND_SPECIAL_HI, ridley_upb_status_pre)
        .status(End, *FIGHTER_STATUS_KIND_SPECIAL_HI, ridley_upb_status_end)
		.game_acmd("game_specialhi", ridley_upb_acmd, Default)
		.effect_acmd("effect_specialhi", ridley_upb_acmd_fx, Default)
		.game_acmd("game_specialairhi", ridley_upb_air_acmd, Default)
		.effect_acmd("effect_specialairhi", ridley_upb_air_acmd_fx, Default)
